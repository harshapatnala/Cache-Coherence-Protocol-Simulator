...
void MESI::PrRd(ulong addr, int processor_number) {
    ...
    if (line == NULL || line->get_state()==I) {
          ...
          if(dir_line-> get_state()==EM ){I2E++;newline->set_state(E);}
          else if(dir_line-> get_state()==S_ ){I2S++;newline->set_state(S);}
...
void MESI::Inv(ulong addr) { 
	...
	cache_line * line = find_line(addr);
        dir_line = directory->find_dir_line(tag_field(addr));
        if(line->get_state()==M || line->get_state()==E || line->get_state()==S){
          line->set_state(I);
